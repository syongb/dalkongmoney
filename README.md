...
현재는 Phase 3까지 구현되어 있습니다. 계정 생성, 로그인, 가계부 생성, 일회용 배우자 초대, household 단위 보안 정책과 거래 등록·조회·수정·삭제를 포함합니다. 실시간 동기화와 예산 화면은 아직 없습니다.
...
8. 금액과 카테고리만 선택해 거래를 저장합니다.
9. `나 / 배우자 / 공동`으로 소비자를 바꿔 각각 저장합니다.
10. 상호명을 입력했을 때 이전의 정확히 같은 상호에서 카테고리가 제안되는지 확인합니다.
11. 거래 한 줄을 눌러 수정하고, 삭제 확인창에서 취소와 삭제를 각각 확인합니다.
