...
export function DisplayNameForm({ displayName, householdName }: { displayName: string; householdName: string }) {
  ...
  return (
    <form action={action} className="space-y-2.5">
      ...
      <label className="block text-xs font-medium">
        가계부 이름
        <input name="household_name" required maxLength={60} defaultValue={householdName} className="mt-1 min-h-10 w-full rounded-lg border border-stone-300 bg-white px-2.5 text-sm outline-none focus:border-stone-700" />
      </label>
      {state.message && (
        <p
          role="status"
      ...
      <button
        disabled={pending}
        className="min-h-10 w-full rounded-lg bg-stone-900 text-sm font-semibold text-white disabled:opacity-50"
      >
        {pending ? "저장하는 중…" : "저장"}
      </button>
    </form>
