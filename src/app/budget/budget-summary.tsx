  ...
  return (
    <li className="border-t border-stone-100 py-2 first:border-t-0">
      <div className="flex items-center justify-between gap-1.5 text-xs">
        ...
        <span className={`shrink-0 rounded px-1.5 py-0.5 font-bold ${style.text} bg-current/10`}>{rate === null ? "0원" : `${rate}%`} · {status}</span>
      </div>
      <div className="mt-1.5 flex items-center gap-2">
        ...
        {budget === 0 ? (
          <p className={`text-[0.68rem] font-semibold ${style.text}`}>{spent > 0 ? `${won.format(over)}원 초과` : "예산 0원"}</p>
        ) : over > 0 ? (
...
export function BudgetSummary({ month, summary }: { month: KoreaMonth; summary: MonthlyBudgetSummary }) {
  ...
  return (
    <section className="mt-3 rounded-lg bg-white p-3 shadow-sm">
      <h2 className="font-bold">{month.monthLabel}</h2>
      ...
      <Link href="/budget?returnTo=%2F%3Fview%3Dbudget" className="mt-3 flex min-h-11 items-center justify-center rounded-lg border border-stone-900 text-sm font-semibold">예산·카테고리 설정</Link>
    </section>
  );
}
